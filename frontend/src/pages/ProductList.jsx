...
export default function ProductList({ category }) {
  ...
  useEffect(() => { setSelectedCategory(category || ""); setSubCategory(""); }, [category]);

  useEffect(() => {
    if (!selectedCategory) { setSubOptions([]); return; }
    api.get("/products", { params: { category: selectedCategory, status: "Active" } })
      .then((r) => setSubOptions([...new Set(r.data.map((p) => p.subCategory).filter(Boolean))]))
      .catch(() => setSubOptions([]));
  }, [selectedCategory]);

  useEffect(() => {
    api.get("/products", { params: { category: selectedCategory, subCategory: subCategory || undefined, search, status: "Active" } })
      .then((r) => setItems(r.data))
      .catch(() => setItems([]));
  }, [selectedCategory, subCategory, search]);
    ...
    <section className="min-h-screen bg-[#f8f7ff] pb-24">
      ...
      <div className="relative overflow-hidden bg-gradient-to-br from-violet-600 via-purple-600 to-indigo-700 px-4 py-6 md:py-10">
        ...
        <div className="relative mx-auto max-w-7xl">
          ...
          <p className="mt-1 text-sm text-white/60">{filteredItems.length} products found</p>
